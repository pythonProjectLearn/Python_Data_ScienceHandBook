...
def train_sparse_data_generate(train_data, fields_dict):
    ...
    with open('utility/train_sparse_data_frac_0.01.pkl','wb') as f:
...
def test_sparse_data_generate(test_data, fields_dict):
    ...
    for data in test_data:
        ...
        if ibatch % 200 == 0:
            logging.info('{}-th batch has finished'.format(ibatch))
    with open('utility/test_sparse_data_frac_0.01.pkl','wb') as f:
        pickle.dump(sparse_data, f)
...
if __name__ == '__main__':
    ...
    fields = ['hour', 'C1', 'C14', 'C15', 'C16', 'C17', 'C18', 'C19', 'C20', 'C21',
              ...
              'device_conn_type']
    batch_size = 512
    train = pd.read_csv('utility/train_frac_0.01.csv', chunksize=batch_size)
    test = pd.read_csv('utility/test.csv', chunksize=batch_size)
    # loading dicts
    fields_dict = {}
    for field in fields:
