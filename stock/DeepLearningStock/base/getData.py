...
sys.path.append(os.path.dirname(os.path.dirname(basedir)))
from postgresql import Postgresql


def get_factors(index, Open, Close, High, Low, Volume, rolling=26, drop=False, normalization=True):
    """
    #
    :param index: 时间
    :param Open:
...
def make_label(df, rolling=30):
    # 对收盘价进行预测,以30天作为窗口,构造上下区间,以2天的差分比来判断价格的波动
    # 如果高于上区间,则上涨,如果低于下区间,则下跌,在中间,则平稳;将问题转化为分类问题
    df['returns'] = df['last_price'].shift(-2) / df['last_price'] - 1.0
    df['upper_boundary'] = df['returns'].rolling(rolling).mean() + 0.5 * df['returns'].rolling(rolling).std()
    df['lower_boundary'] = df['returns'].rolling(rolling).mean() - 0.5 * df['returns'].rolling(rolling).std()
    df.dropna(inplace=True)
    df['labels'] = 1
    df.loc[df['returns'] >= df['upper_boundary'], 'labels'] = 2
    df.loc[df['returns'] <= df['lower_boundary'], 'labels'] = 0
    return df
...
if __name__=='__main__':
    ...
    result = ps.selectSql("select update_time ,open_price, high_price, low_price, last_price, transaction_stocks from stocks_sh_sz where code='600689.SH'")
    df = pd.DataFrame(result, columns=['datetime_stock','open_price', 'high_price', 'low_price', 'close_price', 'volume'])
    df = df.set_index('update_time')

    # 对收盘价进行预测,以30天作为窗口,构造上下区间,以2天的差分比来判断价格的波动
    # 如果高于上区间,则上涨,如果低于下区间,则下跌,在中间,则平稳;将问题转化为分类问题
    targets = make_label(df=df, rolling=30)
    factors = get_factors(index=df.index, Open=df['open_price'].values, Close=df['last_price'].values, High=df['high_price'].values, Low=df['low_price'].values, Volume=df['volume'].values)
