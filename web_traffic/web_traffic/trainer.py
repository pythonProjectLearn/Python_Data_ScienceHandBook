...
def train(name, hparams, multi_gpu=False, n_models=1, train_completeness_threshold=0.01,
          seed=None, data_path=None, logdir=None, max_epoch=100, patience=2, train_sampling=1.0,
    ...
    with tf.device("/cpu:0"):
        inp = VarFeeder.read_vars(f'{data_path}/var')
        if side_split:
    ...
    if save_best_model or save_from_step:
        saver_path = f'{data_path}/cpt/{name}'
...
def predict(checkpoints, hparams, return_x=False, verbose=False, predict_window=6, back_offset=0, n_models=1,
    ...
    with tf.variable_scope('input') as inp_scope:
        with tf.device("/cpu:0"):
            inp = VarFeeder.read_vars(f"{data_path}/vars")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',default='/home/zt/Documents/Data/kaggle_web_traffic/data', help='train data abspath')
    ...
    parser.add_argument('--logdir', default='/home/zt/Documents/Data/kaggle_web_traffic/data/logs', help="Directory for summary logs")
    ...
    # preds = predict(f'{param_dict['data_path']}/cpt/fair_365-15428', 380, hparams, verbose=True, back_offset=60, n_models=3)
    # print(preds)
    """
